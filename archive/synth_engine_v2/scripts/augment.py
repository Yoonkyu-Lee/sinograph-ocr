...
def motion_blur(img, rng, kernel=1, angle=0.0):
    ...
    # PIL.ImageFilter.Kernel only supports 3×3 or 5×5; clamp to 5 for safety.
    k = min(k, 5)
    if k <= 1:
        return img
...
# ---------- deformation (Simard 2003) ----------------------------------------


def elastic(img, rng, *, alpha=10.0, sigma=4.0):
    """Elastic distortion (Simard et al., 2003).

    Generates a random displacement field, smooths it with a Gaussian kernel,
    then normalizes so the *maximum* per-pixel displacement equals `alpha`
    pixels. Produces natural handwriting-like non-rigid warp WITHOUT needing
    stroke structure. Works on any bitmap.

    alpha: max displacement magnitude in pixels (after smoothing + normalization)
    sigma: Gaussian smoothing sigma. Higher = smoother/more coherent warp.
           Typical handwriting-like: sigma in [4, 8].
           Too small → result looks like noise; too large → near-rigid warp.
    """
    from scipy.ndimage import gaussian_filter, map_coordinates
    a = float(_sample(alpha, rng))
    s = float(_sample(sigma, rng))
    if a <= 0:
        return img
    ...
    # Random field in [-1, 1]
    dx = rng.random((h, w)) * 2 - 1
    dy = rng.random((h, w)) * 2 - 1
    if s > 0:
        dx = gaussian_filter(dx, sigma=s, mode="reflect")
        dy = gaussian_filter(dy, sigma=s, mode="reflect")

    # Normalize so the largest displacement equals alpha pixels.
    max_mag = max(float(np.abs(dx).max()), float(np.abs(dy).max()), 1e-6)
    scale = a / max_mag
    dx = dx * scale
    dy = dy * scale
    ...
    if arr.ndim == 3:
        ...
        for c in range(arr.shape[2]):
            result[..., c] = map_coordinates(arr[..., c], coords, order=1, mode="reflect")
    else:
...
OPS: dict[str, Callable[..., Image.Image]] = {
    ...
    # deformation (handwriting-like warp without stroke info)
    "elastic": elastic,
}
...
def apply_pipeline(img: Image.Image, pipeline: list[dict[str, Any]],
    ...
    """[v1-style standalone driver] Apply a sequence of op dicts.

    Each dict has 'op' + op-specific params. Optional 'prob' gates per-sample
    execution. Kept for standalone library usage; v2 pipeline uses the
...
# ---------- v2 layer registration --------------------------------------------
# Wrap each op as a v2 layer operating on Context.canvas, under the name
# "augment.<op_name>". Done here so augment's 24 ops become first-class
# layers in the pipeline registry without any duplication.

try:
    from pipeline import register_layer  # type: ignore[import-not-found]

    def _wrap_op_as_layer(op_fn):
        def layer_fn(ctx, **params):
            ctx.canvas = op_fn(ctx.canvas, ctx.rng, **params)
            return ctx
        return layer_fn

    for _op_name, _op_fn in OPS.items():
        register_layer(f"augment.{_op_name}")(_wrap_op_as_layer(_op_fn))
except ImportError:
    # pipeline module not importable (e.g., running augment.py standalone) —
    # skip v2 registration silently.
    pass
