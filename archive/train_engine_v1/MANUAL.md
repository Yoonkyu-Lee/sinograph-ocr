# train_engine_v1 — Manual & Tuning Log

## 개요

Stage 2 (glyph → codepoint 분류기) 학습 엔진. Stage 1 산출물 (`synth_engine_v3/out/42_production_mobile/`) 을 읽어 ResNet-18 을 학습하고 ckpt / ONNX 을 낸다. 상세 설계는 `doc/12_STAGE2_TRAINING_PLAN.md`.
...
| path | 역할 |
|---|---|
| `scripts/00_cpu_smoke.py` | CPU 전 경로 smoke. fake 48 샘플로 train/eval/ckpt/curves 검증 |
| `scripts/20_train.py` | 메인 학습 CLI. yaml + 오버라이드 지원 |
| `scripts/21_eval.py` | ckpt + manifest 로 top-k / family-aware 평가 |
| `scripts/22_export_onnx.py` | ckpt → ONNX (opset 17, dynamo=False) |
| `modules/dataset.py` | `CorpusDataset`, manifest → class_index, split |
| `modules/model.py` | `build_model("resnet18", num_classes)` |
| `modules/train_loop.py` | `train_one_epoch`, `evaluate` (AMP 지원, sysmon 훅) |
| `modules/sysmon.py` | GPU util (백그라운드 샘플러 200 ms) / VRAM / RSS 기록 |
| `modules/family_eval.py` | canonical_v2 SQLite 로 partial-credit metric |
...
--epochs N              # config.train.epochs 덮어쓰기
--batch-size N          # config.train.batch_size
--num-workers N         # config.train.num_workers
--max-classes N         # config.data.max_classes (None=전체)
--input-size N          # config.model.input_size
--amp on|off
--prefetch-factor N
--decode pil|tvio       # config.data.decode (PIL vs torchvision.io.read_image)
--seed N
```
...
```
# S2-A pilot (100 class × 500, 5 epoch, ResNet-18, 128², AMP)
python train_engine_v1/scripts/20_train.py \
  --config train_engine_v1/configs/resnet18_t1_pilot.yaml

# 튜닝 1 epoch trial (격리된 out_dir)
python train_engine_v1/scripts/20_train.py \
  --config train_engine_v1/configs/resnet18_t1_pilot.yaml \
  --run-tag w8_bs256 --epochs 1 --num-workers 8 --batch-size 256
```
